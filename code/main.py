...
#     polar = plant.QMC6310_polar()
    ...
    period_minutes = 10 # The chosen interval time on the Makerverse Nano Power Timer
    file_name = 'log.txt'
    ...
    heading_tvoc = 'TVOC'
    heading_eco2 = 'eCO2'
    data_heading = [heading_time,heading_moisture, heading_pump, heading_voltage, heading_light, heading_temp, heading_pres, heading_humi, heading_aqi, heading_tvoc, ] # The heading that will appear at the top of the log file
    ...
    upload_data = f"plant,sensor_id=RPI2W moisture={soil_moisture},pump={pump_running_seconds},voltage={voltage},lux={lux},temp={temperature_C},pressure={pressure_Pa},humidity={humidity_RH},aqi={AQI.value},tvoc={TVOC},eco2={eCO2.value} {current_time}000000000"
    headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Authorization': f'Token {secrets.INFLUXDB_API_KEY}'}
    print(f"Uploaded data: {upload_data}")
    
    try:
        response = urequests.post(secrets.INFLUXDB_ENDPOINT, headers=headers, data=upload_data.encode())
